...
(function ($, window, document, undefined) {
    ...
    Form.prototype = {
        ...
        _formEles: {
            ...
            'file': function (data, form) {
                ...
                if (data.uploadUrl === undefined) {
                    data.uploadUrl = App.href + "/api/common/uploadFile" + "?animal_disease_token=" + App.token
                } else {
                    if (data.uploadUrl.indexOf("animal_disease_token=") == -1) {
                        if (data.uploadUrl.indexOf("?") != -1) {
                            data.uploadUrl += ("&animal_disease_token=" + App.token);
                        } else {
                            data.uploadUrl += ("?animal_disease_token=" + App.token);
                        }
                    }
            ...
            'image': function (data, form) {
                ...
                if (data.uploadUrl === undefined) {
                    data.uploadUrl = App.href + "/api/common/uploadImage" + "?animal_disease_token=" + App.token
                } else {
                    if (data.uploadUrl.indexOf("animal_disease_token=") == -1) {
                        if (data.uploadUrl.indexOf("?") != -1) {
                            data.uploadUrl += ("&animal_disease_token=" + App.token);
                        ...
                        function () {
                            ...
                            var editor = KE
                                .create('#' + ele.attr("id"),
                                    {
                                        uploadJson: App.href
                                        + '/api/KE/fileUpload?animal_disease_token=' + App.token,
                                        fileManagerJson: App.href
                                        + '/api/KE/fileManager?animal_disease_token=' + App.token,
                                        minWidth: 0,
                                        width: edWith,
                                        height: (ele
        ...
        _initMultiFileUpload: function () {
            ...
            var uploadUrl = App.href + '/api/common/uploadFiles?animal_disease_token=' + App.token;
            ...
            if (files.length > 0) {
                $.each(files, function (i, file) {
                    ...
                    if (uploadUrl.indexOf("animal_disease_token") == -1) {
                        if (uploadUrl.indexOf("?") != -1) {
                            uploadUrl += ("&animal_disease_token=" + App.token);
                        } else {
                            uploadUrl += ("?animal_disease_token=" + App.token);
                        }
                    }
        ...
        _renderMultipleFiles: function (table, fieldName, fileIds) {
            ...
            if (fileInfoUrl.indexOf("animal_disease_token=") == -1) {
                if (fileInfoUrl.indexOf("?") != -1) {
                    fileInfoUrl += ("&animal_disease_token=" + App.token);
                } else {
                    fileInfoUrl += ("?animal_disease_token=" + App.token);
                }
            }
